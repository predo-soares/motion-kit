...
async function readJson<T>(path: string): Promise<T> {
  ...
  try {
    ...
    throw new MotionKitError(`Could not read "${path}"`, "manifest_not_found");
  }

  try {
    return JSON.parse(raw) as T;
  } catch (cause) {
    throw new MotionKitError(`"${path}" is not valid JSON: ${(cause as Error).message}`, "manifest_invalid_json");
  }
}
...
export async function discoverRegistryItems(cwd: string): Promise<RegistrySource> {
  ...
  if (!root.name) throw new MotionKitError(`"${relative(cwd, rootRegistryPath)}" is missing required field "name"`, "registry_invalid");
  if (!root.homepage) throw new MotionKitError(`"${relative(cwd, rootRegistryPath)}" is missing required field "homepage"`, "registry_invalid");
