import type { Framework, MotionKitConfig, PackageManager } from "./types.js";
...
export function validateConfig(config: unknown, configPath: string): MotionKitConfig {
  const issues: string[] = [];
  const report = (message: string) => issues.push(message);

  if (!config || typeof config !== "object" || Array.isArray(config)) {
    throw new MotionKitError(
      `Invalid motion-kit.json at ${configPath}: expected a JSON object.\n\nNext: Run \`motion-kit init\` to create a valid config.`,
      "invalid_config",
    );
  }
  ...
  if (issues.length > 0) {
    throw new MotionKitError(
      `Invalid motion-kit.json at ${configPath}:\n${issues.map((issue) => `  - ${issue}`).join("\n")}\n\nNext: Run \`motion-kit init --dry-run\` to see a valid config, or edit the file manually.`,
      "invalid_config",
    );
  }

  return value as unknown as MotionKitConfig;
}
