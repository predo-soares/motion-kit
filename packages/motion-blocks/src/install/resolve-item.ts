...
async function loadFromPath(path: string): Promise<RegistryItem> {
  if (!existsSync(path)) {
    throw new MotionBlocksError(
      `Registry item not found: ${path}\n\nNext: Run \`motion-blocks list\` to see available items, or check the name and your \`registry\` setting in motion-blocks.json.`,
      "item_not_found",
    );
  }
...
async function loadFromUrl(url: string): Promise<RegistryItem> {
  ...
  if (!response.ok) {
    throw new MotionBlocksError(
      `Failed to fetch registry item from ${url}: ${response.status} ${response.statusText}\n\nNext: Confirm the item exists with \`motion-blocks list\` and that your \`registry\` URL is correct.`,
      "fetch_failed",
    );
  }
