...
export function resolveCatalogSource(config: MotionBlocksConfig, cwd: string): string {
...
async function loadCatalogFromPath(path: string): Promise<RegistryCatalog> {
  if (!existsSync(path)) {
    throw new MotionBlocksError(`Registry catalog not found: ${path}`, "catalog_not_found");
  }
  ...
  if (!Array.isArray(catalog.items)) {
    throw new MotionBlocksError(`Invalid registry catalog at ${path}: missing "items" array`, "invalid_catalog");
  }
...
async function loadCatalogFromUrl(url: string): Promise<RegistryCatalog> {
  ...
  try {
    ...
    throw new MotionBlocksError(`Failed to fetch registry catalog from ${url}: ${message}`, "fetch_failed");
  }
